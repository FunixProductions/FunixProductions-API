...
public abstract class TwitchReferenceService {
    ...
    protected ApiException handleFeignException(final FeignException e) throws ApiBadRequestException {
        ...
        switch (code) {
            ...
            case 401 -> throw new ApiUnauthorizedException(String.format("Votre token d'accès Twitch est invalide. Erreur: %s", errorMessage), e);
            case 403 -> throw new ApiForbiddenException(String.format("Accès refusé à la ressource Twitch. Erreur: %s", errorMessage), e);
            case 404 -> throw new ApiNotFoundException(String.format("Ressouce Twitch introuvable. Erreur: %s", errorMessage), e);
